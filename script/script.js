...
let myNewBook02 = new Book('book02', 'No Sufoco', 'Chuck Palahniuk', 'Ed. 02', 'R$18.20', 'Depois de descobrir que todas as coisas podem dar errado, sua vida se torna menos sobre viver e mais sobre esperar.', 147);
...
let myNewBook04 = new Book('book04', 'Sobrevivente', 'Chuck Palahniuk', 'Ed. 03', 'R$32.70', 'Uma garota liga e pergunta: \"Dói muito morrer?\" \"Bem, querida\", digo a ela, \"sim, mas dói muito mais continuar vivendo\".', 183);
...
let myNewBook06 = new Book('book06', 'Snuff', 'Chuck Palahniuk', 'Ed. 05', 'R$31.90', 'O que você faz quando toda a sua identidade é destruída em um instante? Como você lida quando toda a sua história de vida acaba errada?', 79);
...
let myNewBook09 = new Book('book09', 'O Cheiro do Ralo', 'Lourenço Mutarelli', 'Ed. 02', 'R$21.50', 'Vinha um forte cheiro de fossa que subia do ralo e invadia meu nariz. Invadia a sala toda. Cheiro de merda, é do ralo, afirmei. Acho que fiquei com vergonha de que ele pensasse que o cheiro vinha de mim.', 107);
...
let populateTable = (bookCollection) => {
    ...
    bookCollection.categories.forEach((e) => {
        htmlOutput += `<td>${e}</td>`
    });
    htmlOutput += `</tr>`

    bookCollection.forEach((e) => {
        htmlOutput += `<tr>`
        for (let i = 0; i < bookCollection.categories.length; i++) {
            htmlOutput += `<td>${e.bookInfo[Object.getOwnPropertyNames(e.bookInfo)[i]]}</td>`
        }
        htmlOutput += `</tr>`
    })
...
div1.innerHTML = populateTable(myBooks);
